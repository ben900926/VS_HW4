...
def draw_boxes(img, bbox, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        total_people = max(total_people, id)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    s += '%g %ss, ' % (n, names[int(c)])  # add to string (e.g. 7 persons)
            ...
            if save_vid:
                ...
                ## modify: mp4 to ts and m3u8 (https://gist.github.com/lukebussey/4d27678c72580aeb660c19a6fb73e9ee)
                if(frame_idx > 0 and frame_idx % 10 == 0):
                    m3u8_path = os.path.join(out, str(frame_idx))

                    vid_writer.release()
                    command = f"ffmpeg -i {save_path}.mp4 -codec: copy -start_number 0 \
                        -hls_time 10 -hls_list_size 0 -f hls {m3u8_path}.m3u8"
                    os.system(command)
